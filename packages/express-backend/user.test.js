...
describe("User Service Tests", () => {
  ...
  test("updateUserById should update the user details correctly", async () => {
    ...
    const updatedUser = await userService.updateUserPointsById(user._id, -25);
  ...
  test("updateUserById should handle non-existing user", async () => {
    ...
    await expect(
      userService.updateUserPointsById(nonExistingId, 50)
  ...
  test("signupUser should create a new user", async () => {
    ...
    const savedUser = await userService.signupUser(
      newUser.email,
      newUser.username,
      newUser.password,
      newUser.firstName,
  ...
  test("should respond with 401 if JWT is invalid", () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        message: expect.stringContaining("JWT error:"),
      })
    );
  });
